...
class DicomRenderer {
    ...
    async renderImage(imageData, viewport) {
        ...
        try {
            ...
            if (renderTime > 500) {
                console.warn(`⚠️ Image rendered in ${renderTime.toFixed(2)}ms (needs optimization)`);
            }
            ...
            try {
                await this.renderFallback(imageData);
            } catch (fallbackError) {
    ...
    async renderFallback(imageData) {
        ...
        // Try to load and display image directly
        if (imageData && (imageData.dataUrl || imageData.url)) {
            const img = new Image();
            img.onload = () => {
